...
A nice neat pattern. We can delete the material from the top of page one, and then loop through the remainder of the pages.
...
We can see that the last 4 rows have the footer, but it's hard for us to work with, since the original row indexes are lost...the new object is renumbered!


The footers are in lines 11 through 14.
...
It would be possible to sort out a programmatic solution to this, but the document is only 10 pages long, so I'm going to make a guess that it's going to be faster for me to do a visual check as to the number of footer rows, and then hard-code a length element to read into a function.

One of the reasons for creating a function for the footer rows of the page is the length() and dplyr::last() functions don't support arithmetic. That is to say, `length(x) - 1` will return an error.

```{r}

length(vc_text_split) - 1
...
#  vec_length <- length(vect)
#  last_x_line <- vec_length - line_nums + 1
#  vect[last_x_line:vec_length]
#}


last_x_line_fun <- function(vect, line_nums){
  last_x_line <- length(vect) - line_nums + 1
  vect[last_x_line:(length(vect))]
}
...
So let's make a clean page one, and then pages 2 through 9.
...
end_linestrip_fun <- function(vect, line_nums){
  ...
  last_x_line <- vic_vec_length - line_nums + 1
  vect[-(last_x_line:vic_vec_length)]
}
